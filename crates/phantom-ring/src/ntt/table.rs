...
fn primitive_root_of_order(order: usize, modulus: u64) -> Result<u64> {
    ...
    for candidate in 2..modulus.min(SEARCH_LIMIT) {
        if pow_mod(candidate, order_u64, modulus) != 1 {
            continue;
        }
        if pow_mod(candidate, order_u64 / 2, modulus) == 1 {
            continue;
        }
        return Ok(candidate);
    }

    let cofactor = (modulus - 1) / order_u64;
    for candidate in 2..modulus.min(SEARCH_LIMIT) {
        let h = pow_mod(candidate, cofactor, modulus);
        if pow_mod(h, order_u64, modulus) != 1 {
            continue;
        }
        if pow_mod(h, order_u64 / 2, modulus) == 1 {
            continue;
        }
        return Ok(h);
    }
