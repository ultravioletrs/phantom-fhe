//! BFV collective Galois (rotation) key generation - real, RNS-hybrid,
//! additive n-of-n.
//!
//! Real BFV rotation keys are the RNS-hybrid `KeySwitchKey` construction
...
//! comment for why). The collective version of that RNS-hybrid
//! construction is derived once, scheme-agnostically, in
//! `common::hybrid` (which see for the full derivation) - this module is a
//! thin wrapper supplying BFV's own `RlweParams` and handling session/
//! replay/wire concerns, mirroring `mpbgv::gkg`'s own shape.
...
/// Collective Galois-key generation helper for BFV. Scoped to one rotation
/// `element` per session, matching `super::CollectiveKeyGen`'s own "one
/// clear thing per session" shape - a caller needing several rotation
/// amounts runs several sessions.
#[derive(Clone, Debug)]
...
impl GaloisKeyGen {
    ...
    /// `KeyGenerator::generate_hybrid_galois_key` itself takes) - sizing it
    /// correctly is the caller's own concern, exactly as it is for the
    /// single-party real path.
    pub fn new(
        params: BfvParams,
        session: SessionState,
        element: usize,
    ...
    /// rotation key. Sums *every* collected share's own `b_j`
    /// (`ShareAggregator::all_shares`, not `aggregate`), verifying (not
    /// summing) each row's own shared `a_j` matches exactly - see
    /// `mpbgv::gkg`'s own doc comment for why summing a shared constant
    /// instead would be a real, previously-found bug class.
