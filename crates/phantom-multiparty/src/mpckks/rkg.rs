//! CKKS collective relinearization-key generation - real, RNS-hybrid,
//! three-round.
//!
//! Real CKKS relinearization keys are the RNS-hybrid `KeySwitchKey`
//! construction (`phantom_lattice::rlwe::KeyGenerator::generate_hybrid_relinearization_key`) -
//! CKKS is, per its own doc comments, actually the scheme this
//! technique was built for (unlike BGV, which needs its own bespoke
//! classical-gadget-decomposition construction instead). The collective
//! version is derived once, scheme-agnostically, in `common::hybrid`
//! (which see for the full derivation) - this module is a thin wrapper
//! supplying CKKS's own `RlweParams` and handling session/replay/wire
//! concerns, mirroring `mpbfv::rkg`'s own identical wiring.
//!
...
/// Collective relinearization-key generation helper for CKKS. See this
...
impl RelinearizationKeyGen {
    ...
    /// advance. `session` is round 0's (the `QP`-collective-public-key
    /// round's) session.
    pub fn new(params: CkksParams, session: SessionState, p_moduli: Vec<Modulus>) -> Self {
        Self {
            params,
            session,
            p_moduli,
    ...
    /// aggregate rows - see `common::hybrid`'s own doc comment.
...
/// Sums every collected share's own row sequence componentwise - see
/// `mpbfv::rkg`'s own identical helper's doc comment.
