...
#[test]
fn collective_public_key_encrypts_decryptable_bfv_ciphertexts() {
    let params = params();
...
fn partial_decryption_reconstructs_signed_bfv_plaintext() {
    ...
    assert_eq!(
        &encoder.decode_i64(&reconstructed).unwrap()[..4],
        &[-2, 3, 5, -6]
    );
}
...
#[test]
fn reencryption_from_bfv_shares_preserves_plaintext() {
    let params = params();
    let ctx = BfvContext::new(params.clone());
    let mut rng = rng();
    let keys = ctx.keygen().unwrap().generate_keypair(&mut rng).unwrap();
