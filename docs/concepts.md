...
- **The error must be genuinely random and correctly distributed** (e.g. discrete Gaussian) — too small, structured, or predictable, and the "hardness" disappears. [`sample_discrete_gaussian`](#sampling) now draws from a real discrete Gaussian rather than a uniform-range placeholder, and every scheme's real (non-transparent) encryption and key-generation path samples genuine error with it; it is not yet constant-time, though (see [`technical-manual.md`](technical-manual.md#ring-internals)) — that remains a real, not cosmetic, security gap today.
...
2. **EvalMod** — homomorphically evaluate the modular reduction that decryption implicitly performs, which is the step that actually "refreshes" the noise/level budget. This is the hardest step in real CKKS bootstrapping (it needs a good polynomial approximation of a periodic function); it now has both a transparent scaffold and a real, homomorphically-evaluable polynomial version — see [`technical-manual.md#bootstrapping-internals`](technical-manual.md#bootstrapping-internals) for what each stage does today, on both paths.
