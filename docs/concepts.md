...
- **The error must be genuinely random and correctly distributed** (e.g. discrete Gaussian) — too small, structured, or predictable, and the "hardness" disappears. [`sample_discrete_gaussian`](#sampling) now draws from a real discrete Gaussian rather than a uniform-range placeholder, but it is not constant-time and no scheme wires it into encryption noise yet (see [`technical-manual.md`](technical-manual.md#ring-internals)) — those remain real, not cosmetic, security gaps today.
