...
**Not yet done:** `Ring::schoolbook_mul` — used throughout `phantom-lattice` for ciphertext multiplication — is still the O(N²) path; nothing above `phantom-ring` calls the NTT backend for multiplication yet (see [Ring internals](#ring-internals) in the performance section below). No hand-unrolled hot loops, no lazy/deferred reduction between butterfly stages (still calls `mul_mod`/`add_mod`/`sub_mod` per operation, not `BarrettReducer`), and no Montgomery-form precomputed root table — all real further-optimization opportunities, not correctness gaps.
...
- `phantom_ring::ntt::CpuNttBackend` is now a real O(N log N) transform (see [Ring internals](#ring-internals) above) — measured ~414µs for a forward+inverse round trip at `N=1024` (`crates/phantom-benches/benches/ring_ntt.rs`, 100 iterations combined) — but still unoptimized (no lazy reduction between butterfly stages, no Montgomery-form root table, no unrolling).
- `Ring::schoolbook_mul` (O(N²) negacyclic multiplication) is what `phantom-lattice` actually uses for ciphertext multiplication today — the real NTT above isn't wired into it yet, so this remains the active bottleneck for anything built on `phantom-lattice`/`phantom-schemes`, not the NTT itself.
