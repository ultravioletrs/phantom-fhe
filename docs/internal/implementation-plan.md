...
2. [In progress] Connect BGV/BFV/CKKS circuit evaluators to real evaluator semantics. Scoping this properly - before writing any circuit code - surfaced that the three schemes are in genuinely different starting positions, not a single "port the evaluators" task.
   ...
   **CKKS: the ring arithmetic already *is* SIMD (canonical embedding), but multi-level circuits exposed a real gap.** Unlike BGV/BFV, CKKS's real ring multiplication genuinely does correspond to elementwise slot multiplication (the whole point of the canonical embedding - already confirmed by Workstream 5's own cross-operation pipeline test, `mul_real` producing the elementwise product). A single multiply-relinearize-rescale step was verified correct here too before building anything further. But extending that into a genuine multi-step (Horner-method) circuit exposed two real, previously-undiscovered gaps:
   ...
   - **Found, not yet fixed**: `Scale::compatible`'s `1e-9` relative tolerance (used by every real-path `add_real`/`mul_real`/`add_plain_real` scale-match check) is tight enough that it can reject two ciphertexts that are legitimately at "the same" scale after a real rescale - a dropped modulus is only ever *close to* `2^scale_bits` (it must be odd, per `Modulus::new`, so it can never equal an even power of two exactly), so scale drifts by a small absolute amount on every rescale; that drift compounds across a multi-step circuit and can exceed the current tolerance even though nothing is actually wrong. Reproduced directly: a two-step Horner chain (`x^3` via `x^2` then `*x`) failed `add_real`'s own scale check despite both operands decrypting correctly. Not yet fixed here - the right fix needs its own derivation (likely comparing `log2(scale)` against an absolute bit-tolerance that can be reasoned about per rescale step, rather than a single fixed relative tolerance chosen without reference to real rescale drift) and its own verification before touching a check every real CKKS arithmetic method relies on.

   Net effect: `ckks::Evaluator` now has one more real building block (`drop_level_real`) that a future multi-level circuit evaluator will need, and the actual blocker for building one (the `Scale::compatible` tolerance) is now identified precisely rather than discovered mid-implementation of something bigger. No `phantom-circuits` evaluator has been touched yet for any of the three schemes - genuinely still item 2's own open task, now scoped accurately instead of optimistically.
