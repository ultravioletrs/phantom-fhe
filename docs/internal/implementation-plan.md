...
Current status: Alpha Hardening Workstream 11 (CKKS real bootstrapping: modulus raise) landed a real, tested `Evaluator::raise_level_real`, a widened real-domain `EvalMod::reduce_mod_q_real_wide`, a genuinely complex-valued `EvalMod::reduce_mod_q_real_complex_wide`, and - the workstream's largest single piece - a from-scratch, paper-verified redesign of `CoeffsToSlots::apply_real`/`SlotsToCoeffs::apply_real` matching real CKKS bootstrapping's own CoeffToSlot/SlotToCoeff construction (Cheon-Han-Kim-Kim-Song, EUROCRYPT 2018), confirmed correct against real encryption (not just transparently). What remains is narrower than it was: not an architectural mismatch anymore, but at least one more encoding-precision issue in `EvalMod::reduce_mod_q_real_wide`'s own arithmetic at the magnitude a real `raise_modulus` actually has (`~2^32`, not the `~100`-scale value every passing test still uses) - one such issue (a plaintext constant silently rounding to zero during encoding) was found and fixed along the way, but a full real end-to-end attempt still doesn't recover the message, evidenced by a large spurious imaginary component this session did not isolate. See Workstream 11's own entry (item 7) for the full derivation and exactly what's left. Workstreams 1 (Phase 0 cleanup), 2 (scaffold boundary and API honesty), 3 (ring and RNS foundation hardening, item 5b intentionally deferred, see its own entry), 4 (production RLWE/RGSW track), 5 (production BFV/BGV/CKKS track), and 6 are done - see below. Workstream 6's own item 1 (keeping circuit planning APIs stable) is an ongoing invariant, not a one-time task, and stays in force going forward the same way analogous "stay stable" items do in earlier workstreams. Workstream 7 (multiparty and protocol security) is next once Workstream 11's remaining item is either finished or explicitly deferred.
...
- The wraparound-integer bound `EvalMod` relies on is derived, not assumed. **Met** - and the derivation surfaced that `EvalMod`'s own domain doesn't yet cover a realistic secret (addressed by item 5's own domain-widening) or a genuinely complex value (addressed by item 6's own complex extension), which in turn surfaced the deeper `CoeffsToSlots`/`raise_level_real` domain-mismatch (addressed by item 7's own redesign, verified against real encryption) and a real-`raise_modulus`-scale encoding-precision bug (item 7's own two-step pre-scale fix, partial).

**Not met, tracked as a further, separate item**: `Bootstrapper::bootstrap_real_wide` running end to end starting from a genuinely-exhausted real ciphertext at a realistic `raise_modulus` magnitude (not a test-constructed, engineered one, and not the toy `raise_modulus=100` every passing test in this file still uses) - blocked on at least one more encoding-precision issue in `EvalMod::reduce_mod_q_real_wide` beyond the pre-scale step item 7's own fix already found, evidenced by a large spurious imaginary component in its output for a real-scale `raise_modulus` that item 7's own attempt did not isolate. The architectural piece (item 7's own `CoeffsToSlots`/`SlotsToCoeffs` redesign) is done and verified; what remains is narrower in scope - hardening `EvalMod`'s own arithmetic for the actual magnitude a real `raise_modulus` has, not a further redesign. Revisit once that precision issue is found.
