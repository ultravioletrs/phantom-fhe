...
4. Implement production relinearization (`Evaluator::relinearize` is currently a literal identity function, not a real degree reduction). Item 3's `generate_key_switch_key`/`key_switch` are ready to build this on - what's still needed is specifically the `s_old = s²` case: a genuine centered CRT lift of `s²` (computed via `ring.mul(s, s)` in `Q`-basis) into the extended `QP` ring, since `s²`'s coefficients aren't ternary the way `rebase_ternary_secret` assumes (that helper only handles the simpler ternary-to-ternary case item 3's own tests use).
...
- RLWE operations are no longer transparent/toy: encryption and key generation now sample real noise (item 1), and RGSW encryption/external product are now a real gadget-matrix construction rather than plaintext-backed (items 7, 8) - relinearization, key switching, and repacking remain identity-function placeholders (items 3, 4, 6), so not yet fully met.
- Key switching and relinearization have meaningful cryptographic behavior. Not yet met (items 3, 4).
- Higher scheme crates can build production BFV/BGV/CKKS semantics on top. Not yet met - depends on the above.
