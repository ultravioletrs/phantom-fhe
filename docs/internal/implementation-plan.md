...
- The wraparound-integer bound `EvalMod` relies on is derived, not assumed. **Met** - and the derivation surfaced that `EvalMod`'s own domain doesn't yet cover a realistic secret, tracked below as the concrete remainder.

**Not met, tracked as a further, separate item**: `Bootstrapper::bootstrap_real` running end to end starting from a genuinely-exhausted real ciphertext (not a test-constructed, pre-raised one) - blocked on widening `EvalMod::reduce_mod_q_real`'s domain to match a real secret's Hamming weight, which item 3's own note above scopes but doesn't attempt. Revisit once that domain-widening derivation is done.
