...
Current status: Alpha Hardening Workstream 11 (CKKS real bootstrapping: modulus raise) is **done**. It landed a real, tested `Evaluator::raise_level_real`, a widened real-domain `EvalMod::reduce_mod_q_real_wide`, a genuinely complex-valued `EvalMod::reduce_mod_q_real_complex_wide`, a from-scratch, paper-verified redesign of `CoeffsToSlots::apply_real`/`SlotsToCoeffs::apply_real` matching real CKKS bootstrapping's own CoeffToSlot/SlotToCoeff construction (Cheon-Han-Kim-Kim-Song, EUROCRYPT 2018), and - closing the workstream - a genuine real end-to-end verification (`raise_level_real` through the public `Bootstrapper::bootstrap_real_wide`, recovering a real message from a real raise, not an engineered wraparound). What looked like a further encoding-precision or architectural gap turned out to be neither: it was an incorrect `raise_modulus` value in the test itself (must be `q0/Delta`, not the raw `q0`) plus a stale, pre-redesign doubling-domain bound in a doc comment that never got updated after item 7's own `CoeffsToSlots` redesign made it inapplicable. See Workstream 11's own entry (item 8) for the full root-cause trace. Workstreams 1 (Phase 0 cleanup), 2 (scaffold boundary and API honesty), 3 (ring and RNS foundation hardening, item 5b intentionally deferred, see its own entry), 4 (production RLWE/RGSW track), 5 (production BFV/BGV/CKKS track), 6, 7, 8, 9, 10, and 11 are done - see below. Workstream 6's own item 1 (keeping circuit planning APIs stable) is an ongoing invariant, not a one-time task, and stays in force going forward the same way analogous "stay stable" items do in earlier workstreams. Workstream 7 (multiparty and protocol security) is done in full: item 3 (real transcript hash), item 4a (a standalone, tested Pedersen VSS distributed-key-generation primitive, `phantom-multiparty::vss`), item 4b (real collective public-key generation wired for BGV, `mpbgv::CollectiveKeyGen`), item 4c (real collective Galois/rotation-key generation wired for BGV, `mpbgv::GaloisKeyGen`), item 4d (real collective relinearization-key generation wired for BGV via a genuine two-round protocol, `mpbgv::RelinearizationKeyGen`), item 5b (real collaborative key-switching/PCKS wired for BGV, `mpbgv::ReEncryptor` - additive n-of-n by design, not a step toward Lagrange/Shamir reconstruction, see that item's own text), and item 5a (PCKS's smudging noise replaced with a rigorously derived, cited bound, `phantom_lattice::security::smudging_std_dev`, plus a new large-sigma `phantom_ring::sampling::sample_smudging_gaussian` the existing table-based sampler couldn't handle at that scale), item 5c (real collective decryption wired for BGV, `mpbgv::PartialDecryptor` — the last real placeholder among `mpbgv`'s core protocols, closed out), item 8 (`common::ReplayGuard`, anti-replay protection for `mpbgv`'s five real protocols), and item 9 (BFV slice: `mpbfv::CollectiveKeyGen`/`ReEncryptor`/`PartialDecryptor` mirrored from `mpbgv`, dropping BGV's own `t`-scaling since BFV's real path doesn't need it), item 10 (the matching CKKS slice - same three protocols, plus CKKS-specific level-tracking and a real, previously-undocumented finding that smudging noise costs CKKS decode precision directly, unlike BGV/BFV's exact modular reduction), and item 11 (a new collective RNS-hybrid `GaloisKeyGen`/`RelinearizationKeyGen` construction, derived once in a shared `common::hybrid` module and wired into both `mpbfv` and `mpckks`, closing out the BGV-to-BFV/CKKS mirror in full - all five real protocols now real for all three schemes), item 6 (the multiparty adversary model documented explicitly in `SECURITY.md`), item 7 (adversarial tests for protocol confusion and the shared-constant bug class, closing concrete gaps an audit found in the existing suite), and item 5's own remaining `InteractiveBootstrap` piece (a real, single-round collective-bootstrap construction - the paper's own **ColBootstrap** protocol - for BGV, BFV, and CKKS alike, closing out this item and Workstream 7 as a whole) are all done. Workstream 7 has no open items remaining.

Workstream 8 (testing, serialization, and compatibility) is done in full: item 1 (golden-byte fixture tests for every one of the 16 `SerializationHeader`-based type pairs across `phantom-lattice`/`phantom-schemes`/`phantom-bootstrapping`/`phantom-circuits`, plus `phantom-multiparty`'s own three bespoke-format types - caught a real hand-transcription bug directly, exactly the class of drift this item exists to catch), item 2 (exhaustive domain-mismatch coverage extended to every type pair, not just one representative pair, plus the previously-untested wrong-version rejection case everywhere), item 3 (a `proptest` property per crate that decoding arbitrary bytes never panics), item 4 (audited before adding anything: `phantom-lattice`/`phantom-schemes`/`phantom-bootstrapping` already had extensive hand-seeded randomized coverage, so the new `phantom-circuits/tests/property_tests.rs` targets the genuine, confirmed gap - `common::lintrans`'s own scheme-independent planning logic - instead of duplicating what already existed), item 5 (new `docs/internal/serialization-compatibility-policy.md`), and item 6 (confirmed, not assumed, that no `SecretKey` has ever had an `encode`/`decode` pair, documented explicitly alongside the one deliberate exception, `vss::VssShare`). Also fixed real drift found along the way in `docs/developer-guide.md` and `docs/internal/dependency-policy.md`, both out of sync with the actual state before this item.

Workstream 9 (performance and benchmarking) is done in full: items 1-3 (converted all 10 `phantom-benches` bench binaries from ad hoc `std::time::Instant` smoke timing to Criterion - approved per this file's own prior pre-approval, not reopened here - sharing one CI-tuned configuration so the full `cargo bench -p phantom-benches` run stays under a minute despite being part of `make ci`), item 4 (added `toy`/`small` parameter tiers to the five ring/RLWE-level primitive benches, where scaling is just a modulus/degree choice; deliberately left the five scheme-level evaluator benches single-tier, since a second BGV/BFV/CKKS/bootstrapping/multiparty parameter set is the same "no production preset to graduate to yet" decision `docs/user-guide.md#choosing-parameters` already defers elsewhere, not something to informally invent here - found and fixed a genuine `u128` overflow in `reconstruct_residue`'s per-term reconstruction while building this tier, unrelated to the benchmarking work itself but caught only because a bench now exercises that function at a larger modulus size), and item 5 (a new `phantom_benches::alloc::CountingAllocator`, printing a one-shot allocation-count diagnostic for each of the five evaluator benches, clearly labeled as separate from Criterion's own statistically-sampled timing). Also fixed real drift found along the way in `docs/developer-guide.md`'s "adding a benchmark target" shape, `docs/internal/dependency-policy.md`'s `criterion` bullet, and `README.md`'s now-inaccurate "dependency-free smoke benchmarks" line.

Workstream 10 (Alpha release checklist) is done in full: item 1 (`make ci` passes clean), item 2 (all 16 `phantom-examples` binaries run end to end via `make examples`), item 3 (audited README/implementation-plan/release-checklist against each other and the repo - found and fixed `docs/internal/release-checklist.md`'s Beta section still listing three items Workstreams 8/9 had already closed as open TODOs), item 4 (audited every toy/scaffold warning across `SECURITY.md` and the user-facing docs for staleness in both directions - found and fixed two real, contradictory leftovers in `docs/concepts.md` and `docs/technical-manual.md` claiming the real discrete-Gaussian sampler still "isn't wired into" any scheme's encryption error, false since Workstream 4 item 1, and directly contradicted by the very same `technical-manual.md` document six lines later), and item 5 (new root `CHANGELOG.md` with a `0.1.0 - Alpha` entry covering the implemented crate map, supported examples, security status, known scaffold limitations, and planned production-hardening tracks, linked from README.md). This closes the Alpha Hardening cycle in full - Workstreams 1-9 and 11 done, Workstream 2 item 2 and Workstream 3 item 5b intentionally deferred (see their own entries), Workstream 10 (this checklist) done.
...
2. [Done, one representative preset] Separate toy/test presets from a production-shaped one. This item's own stated blocking gate ("noise-management/RNS/security-review work in Workstreams 3-7 landing first") had actually cleared before this was picked up - all of Workstreams 3-7 were done, but nobody revisited this item after they landed, and it took a direct user question ("why the [...] is this deferred?") to catch that the deferral rationale was stale. The same category of stale-deferral the Workstream 10 doc audit was specifically supposed to catch and didn't (that audit checked for stale *completion* claims, not stale *deferral* rationales - a real gap in how that audit was scoped). Shipped: one concrete, table-validated example (`N=8192`, 3 NTT-friendly ~55-bit `Q` primes totalling ~165 of the table's 218-bit budget, 4 ~58-bit `P` primes for hybrid key-switching and BFV's extended-basis tensor, `t=65537` for BGV/BFV, `2^55` default scale for CKKS), passing `.require_128_bit_security()` and proven correct end to end - real keygen, encrypt, one multiplication, relinearize, (CKKS) rescale, decrypt - for BGV, BFV, and CKKS alike, in `crates/phantom-schemes/tests/production_preset.rs` (`#[ignore]`d by default - minutes-long at this scale on today's unoptimized arithmetic, run explicitly with `--ignored`), documented in `docs/user-guide.md#choosing-parameters`. Deliberately narrow, and stated as such rather than overclaimed: this is one example for one multiplicative depth, not a general production-parameter methodology, not presets for other circuit depths, and not an independent cryptographic security review of the choice (SECURITY.md's TRL 2-3 framing is unchanged by this - it closes "no example to graduate to," not "needs a security audit"). Also an honest, explicitly documented limitation: `.require_128_bit_security()` checks only `Q` against the standard table, with no concept of the auxiliary `P` moduli real hybrid key-switching/`mul_real` need, so the combined `QP` bit-length here hasn't been checked against any published table for an extended-dimension RLWE instance.

   Actually running this preset at real scale surfaced three genuine, previously-undiscovered bugs - none hypothetical, each found by the preset simply refusing to work and root-caused rather than worked around:

   - **A hang in `phantom_ring::ntt::table::primitive_root_of_order`**, not just slowness: it searched for a primitive root by testing candidates `2, 3, 4, ...` directly against the modulus - fine when the modulus stays close in size to `2 * degree` (every existing preset in this codebase), but for a modulus far larger than `2 * degree` (any real multi-prime RNS ciphertext modulus at a realistic, much smaller degree), the order-`2*degree` subgroup shrinks to a vanishingly small, effectively randomly-scattered fraction of `[2, modulus)`, needing on the order of `modulus / (2*degree)` iterations - billions, for a real ~55-bit prime, that never actually completed in any observed run. Fixed by computing a candidate directly via the already-known cofactor `(modulus - 1) / order`, no search needed - see `docs/technical-manual.md#ring-internals`.
   - **A real denial-of-service hazard in `phantom_multiparty::vss::scalar_embed::recover_centered`**, found via an external report (a reviewer building on top of this library) and independently verified against the actual source before fixing, not taken on faith: an `O(magnitude_bound)` linear search over `Scalar` additions, safe only because this crate's own callers happen to pass small bounds - nothing in the public signature (`magnitude_bound: i128`) prevented a much larger one. Fixed to `O(1)` via direct canonical-byte inspection - see `SECURITY.md`'s multiparty section.
   - **A real decode bug in real BFV for any `Q` with more than one modulus** - the deepest of the three, and the actual root cause of this preset's own BFV pipeline failing outright (not just imprecisely) at first. `BatchEncoder::decode_u64_real`/`decode_batched_real` read only `ring().moduli()[0]`, correct only when `Q` happens to be a single modulus (every prior real-BFV test) or when the scaled value is smaller than any single modulus (true for BGV's `m + t*e`, false for BFV's `Delta*m + noise`, which is comparable in size to the *whole* `Q` by construction). Fixed with a new shared primitive, `phantom_ring::rns::decode_scaled_value`, reusing `rescale_and_round`'s own CRT-reconstruction/rounding machinery without needing the auxiliary-basis extension that function's tensor-product use case requires - see `docs/technical-manual.md#bgv--bfv`.

   A fourth apparent issue (BFV's `mul_real` producing wrong results) turned out to be this same decode bug's own symptom, not a separate one, once isolated with a small standalone reproduction rather than debugged through the full pipeline - and a genuinely separate, smaller finding along the way: the auxiliary `P` moduli needed for BFV's `mul_real`/`rescale_and_round` (bounded by `degree * (Q/2)^2`, `rescale_and_round`'s own doc comment) are sized very differently from what hybrid key-switching alone needs (comparable to `Q`'s largest single prime) - an earlier, smaller `P` satisfied the latter but not the former, silently producing wrong (not just imprecise) BFV output until sized to the actual documented bound. CKKS's own precision (originally under a tolerance meant to be generous) improved once its scale was set close to `Q`'s own prime size rather than disconnected from it - a real parameter-tuning lesson (CKKS's rescale divides out exactly one `Q` modulus, so precision is best when that modulus is close in size to the tracked scale), not a bug.
...
Workstream 2 is now effectively complete; item 3 remains intentionally deferred (see above) rather than outstanding work. Item 2 is done as of this session (see above) - no longer deferred.
...
5. [Done] Replaced share aggregation's byte-equality check (`ensure_equal_payloads`) with real cryptographic constructions across `PartialDecryptor`/`ReEncryptor`/`InteractiveBootstrap`, for all three schemes — none of the three ended up needing real threshold secret-share reconstruction (Lagrange interpolation): `PartialDecryptor`/`ReEncryptor` are additive n-of-n (items 5b/5c/9/10, see their own text for why), and `InteractiveBootstrap` turned out to need a third kind of construction again, closing out the item in full.

    **`InteractiveBootstrap`** is now a real, single-round collective-bootstrap protocol for BGV, BFV, and CKKS — the last open piece of this item, and of Workstream 7 as a whole. Unlike CKG/GKG/RKG/PCKS/PartialDecryptor, this wasn't a natural extension of anything already built, and needed real research before design (two first-principles drafts turned out subtly wrong before landing on the right shape - see below). It fuses two already-real constructions in this codebase via a per-participant mask that cancels exactly on combination: `PartialDecryptor`'s own `d_i = c1*s_i + t*e_i` (decrypt-toward-the-null-key share) and `CollectiveKeyGen`'s own `b_i = t*e_i - a*s_i` (fresh-encryption share). This is the real single-round **ColBootstrap** protocol Mouchet, Troncoso-Pastoriza, Bossuat & Hubaux describe for BFV ([eprint 2020/304](https://eprint.iacr.org/2020/304), Protocol 5, Section IV-H, the same paper already cited for this crate's own smudging-noise formula) — extracted directly from the paper's own protocol box and verified numerically (Python, 400/400 trials) before writing any Rust. Each participant discloses one share, `h0_i = c1*s_i + t*e0_i - M_i`, `h1_i = t*e1_i - a*s_i + M_i` (`M_i` a private per-participant mask, `e0_i` smudging-sized off the caller's own `ciphertext_noise_bound`, `e1_i` ordinary fresh noise); combining every share and revealing `c0 + sum(h0_i)` through BGV/BFV's own exact `mod t`/`Delta` decode discards the input ciphertext's own noise *entirely* (the same reason their decode always did, applied here to a masked value instead of the plaintext directly) while hiding the plaintext completely (`sum(M_i)` is never known to anyone) — re-embedding and adding `sum(h1_i)` gives a genuine fresh ciphertext under the *same* collective secret, with noise reset to just `e1`, independent of how noisy the input was.

    CKKS needed a real adaptation, not a mechanical port, discovered through two false starts: a pure-polynomial version of the same algebra doesn't actually discard the input ciphertext's own noise the way BGV/BFV's does, since CKKS has no discrete `mod t` step to discard anything *through* (confirmed by tracing the algebra by hand, not assumed); and sizing the message-hiding mask the same `smudging_std_dev` way `e0_i`/`e1_i` already are overflowed the Gaussian sampler's own realistic range once the "signal" being flooded was a `Delta`-scaled message rather than a noise term (a real numeric failure hit while testing, not a hypothetical). Consulting a reference implementation's own real multiparty-CKKS refresh code (its structure, not its own private source, cited) confirmed the fix for both: no discard step exists or is needed for the plain (non-transforming) case — the input ciphertext's own noise legitimately persists into the output, capping total noise/precision to a known floor (dominated by `e0_i`) rather than eliminating it, consistent with this crate's own "smudging costs CKKS precision directly" finding (item 10) — and the mask should be uniform over the *entire* ciphertext ring (`sample_uniform`, the same primitive `derive_common_ring_element`'s own common value already uses), not a scaled Gaussian: since the ring modulus is astronomically larger than any message's own scaled magnitude, this hides the masked-reveal value completely and needs no security-parameter sizing of its own at all.

    Verified end to end for all three schemes (`phase14_mpbgv.rs`, `phase15_mpbfv.rs`, `phase16_mpckks.rs`): real DKG'd collective key, real encryption, an *artificially elevated* noise level for BGV/BFV (proving the reset, not merely working on an already-fresh ciphertext), a real single-round bootstrap, and a decode recovering the original value (exactly for BGV/BFV; within the expected smudging-dominated tolerance for CKKS). `phantom-examples::mpckks_interactive_bootstrap` needed the same real-DKG rewrite `mpckks_basic` already got, and moved into `tests/examples_run.rs`'s real-parameters group alongside it - the same lesson learned repeatedly this session about checking that crate after any `phantom-multiparty` signature change.

5c. [Done, mpbgv slice] Wired real collective (partial) decryption for `mpbgv::PartialDecryptor` — the last real placeholder among `mpbgv`'s core protocols. Structurally the simplest of the five real `mpbgv` protocols: a special case of item 5b's own PCKS construction, "collective key-switching toward the null key" — the same framing Mouchet, Troncoso-Pastoriza, Bossuat & Hubaux ([eprint 2020/304](https://eprint.iacr.org/2020/304), already cited for item 5a's smudging formula) use for their own analogous protocol ("the KeySwitch protocol can be used as a decryption protocol, `s' = 0`"). Construction: each participant computes `d_i = c1*s_i + t*e_i` (`e_i` smudging-sized, same `phantom_lattice::security::smudging_std_dev` formula item 5a already established — `d_i` is, from anyone's view without the collective secret, an RLWE sample `(c1, c1*s_i + t*e_i)`, hiding `s_i` via RLWE hardness with smudging additionally hiding the exact noise realization); combining every participant's own `d_i` gives `c0 + sum_i(d_i) = c0 + c1*s + t*(sum_i e_i)`, exactly `bgv::Decryptor::decrypt`'s own single-party algebra plus a `t`-scaled term that vanishes under the caller's own subsequent `mod t` decode. Reuses, unchanged, every primitive item 5a/8 already built (`smudging_std_dev`/`sample_smudging_gaussian`/`ReplayGuard`/`ShareAggregator::all_shares`) — no new cryptographic derivation or numerical verification needed, since it's the identical `t`-scaled additive shape every real `mpbgv` protocol this session already established. Additive n-of-n, matching `CollectiveKeyGen`/`GaloisKeyGen`/`RelinearizationKeyGen`/`ReEncryptor`'s own default; genuine `t`-of-`n` threshold decryption remains a separate, larger, not-yet-attempted piece of work, the same carve-out item 5b's own text already states for itself.

    Verified end to end (`phase14_mpbgv.rs::partial_decryption_reconstructs_plaintext`): the same participants who generate a real collective public key (`CollectiveKeyGen`) also collectively decrypt a real ciphertext under it — `aggregate_plaintext`'s own output already *is* the decrypted plaintext, no test-only secret reconstruction needed at all this time (unlike CKG/GKG/RKG/PCKS's own tests), since that's the actual point of partial decryption. `phantom-examples::mpbgv_basic` (the public-facing example, previously transparent) was updated to a real two-participant collective-key-generation-plus-decryption workflow and moved into `tests/examples_run.rs`'s real-parameters test group, since it now needs the same noise-safe parameters every other real BGV path does.
...
6. [Done] Documented the multiparty adversary model explicitly in `SECURITY.md`, consolidating what had previously only been stated piecemeal across separate bullets: every real construction (CKG, GKG, RKG, PCKS, PartialDecryptor, across all three schemes) is additive n-of-n, so confidentiality of the collective secret holds as long as at least one contributor is honest - not active/malicious security. What's mechanically enforced is *format/shape* validation only: the generic `Share`/`ShareAggregator` checks (stale session/round, duplicate/unknown participant, mismatched `ShareKind`), `GaloisKeyGen`'s own shared-constant-consistency check, `RelinearizationKeyGen`'s row-count/shape agreement check, and `ReplayGuard`'s anti-retry protection. Explicitly documented, not fixed, as a genuine, previously-unstated gap: none of this proves (e.g. via zero-knowledge) that a well-formed share was honestly computed from a participant's real secret, so a malicious participant can still submit a well-formed-but-wrong share to silently sabotage correctness (wrong output, not a leak) - undetectable today, and out of scope here (would need share-correctness proofs, materially bigger than this item). `InteractiveBootstrap`'s own status (still fully transparent for all three schemes, no real secret-sharing, so no adversarial model applies to it yet) is stated plainly as a separate, independent gap.
7. [Done] Added adversarial tests closing concrete gaps an audit of the existing suite found before writing anything: `ReplayGuard`'s own rejection behavior had a test only for `mpbgv` despite being wired into all three schemes' five protocols each - mirrored into `phase15_mpbfv.rs`/`phase16_mpckks.rs`. Two "protocol confusion" scenarios that pass the generic `Share`/`ShareAggregator` checks but must still be caught by a scheme's own logic had no coverage at all: a share genuinely produced for one round smuggled into a different round's aggregator (both reuse the identical `ShareKind::RelinearizationKeyGen` tag - only the session's own `round` number disambiguates them; tested for both `mpbgv::rkg`'s two-round and `mpbfv`/`mpckks::rkg`'s three-round variants), and a real `CollectiveKeyGen` share's own payload relabeled under `ShareKind::GaloisKeyGen` for the same session (passes the generic kind check, still rejected once `GaloisKeyGen::aggregate_keys`'s own wire-decode tries to interpret it as row-pairs). Also added, for both the classical (`mpbgv::gkg`) and RNS-hybrid (`mpckks::gkg`) constructions, a regression test for the exact "shared constant vs per-participant contribution" bug class this session already found and fixed once for real (item 4c) - simulated by having one participant compute their own row against the wrong rotation `element`, producing a genuine, well-formed share whose shared constant `a_{j,i}` doesn't match; had zero test coverage before this item. Each new test was verified, not just written: for the two shared-constant regression tests, the underlying check was temporarily removed locally and the test confirmed to fail, before restoring it - proof the test exercises the intended defense rather than passing vacuously. See item 6's own text for the deliberate, documented boundary of what these tests do and don't cover (format/shape attacks, not a malicious-but-well-formed share).
8. [Done] Added `phantom-multiparty::common::ReplayGuard` and wired it into `mpbgv`'s four real protocols at the time (CKG/GKG/RKG's both rounds/PCKS), guarding against a participant's `create_share`/`create_share_round1`/`create_share_round2` being invoked more than once for the same `(session id, round, protocol, participant)` — repeated shares for the same deterministic public randomness and secret-key contribution leak key material via accumulated linear algebra, regardless of per-call smudging noise (see item 5a's own "separate finding"). **Also wired into `PartialDecryptor` when it went real, item 5c** — five real `mpbgv` protocols use it as of that item.

9. [Done, BFV slice - CKG/PCKS/PartialDecryptor only] Mirrored `mpbgv::CollectiveKeyGen`/`ReEncryptor`/`PartialDecryptor` to `mpbfv` - see item 10 for the matching CKKS slice. **Researched before implementing, not assumed uniform**: the five protocols split into two genuinely different-sized jobs. CKG/PCKS/PartialDecryptor port near-mechanically to both BFV and CKKS, since both schemes' real public keys use *plain, unscaled* noise (`b = -(a*s+e), a`) — unlike BGV, neither has a plaintext-modulus-scaled noise trick to replicate (`bfv::keygen`'s own doc comment: "BFV's public key needs no special scaling"). GKG/RKG, by contrast, need a *genuinely new* construction for BFV and CKKS, not a port of `mpbgv::gkg`/`rkg`: those deliberately avoid the RNS-hybrid technique specifically to protect BGV's own exact `mod t` decode (see item 4c's own text) — a constraint BFV/CKKS don't have, since both already use `phantom_lattice::rlwe::KeyGenerator::generate_hybrid_relinearization_key`/`generate_hybrid_galois_key` unmodified for their own real single-party keys. A collective version of *that* construction is separate, not-yet-attempted work (needs its own two-round design for RKG's own `s²` handling, analogous to but structurally different from `mpbgv::rkg`'s gadget-row approach) — deliberately not attempted in this item, tracked as a follow-up.

    Construction: identical to `mpbgv::ckg`/`reencryption`/`partial_decrypt`'s own additive `t`-scaled equations, with the `t`-scaling dropped throughout (`b_i = e_i - a*s_i` instead of `b_i = t*e_i - a*s_i`, and so on for PCKS's `h0_i`/`h1_i` and `PartialDecryptor`'s `d_i`) — no new algebra to verify numerically, since BFV's own real path (`Delta*m + e - a*s`, `Delta = floor(Q/t)` scaling only the *message*, never appearing in any of these three message-free shares) was already established in Workstream 5 and this session's own BGV work already verified the identical additive shape. Reuses every mechanism `mpbgv` already built unchanged: `ReplayGuard`, `ShareAggregator::all_shares`, `phantom_lattice::security::smudging_std_dev`/`sample_smudging_gaussian` for PCKS/PartialDecryptor's own noise, and `phantom_schemes::bfv::noise`'s own composable bound functions (confirmed to already exist with the same shape as `bgv::noise`) for the `ciphertext_noise_bound` parameter. `mpbfv::reencryption`/`partial_decrypt` needed a new `encode_poly_pair`/`decode_poly_pair` pair added to `mpbfv::wire` (already existed for `mpbgv::wire`, PCKS's own two-poly share needed it) - both previously only forwarded a single raw ciphertext component with no secret/noise/`ReplayGuard` parameters at all, essentially a pass-the-ciphertext-through scaffold rather than real crypto, unlike `mpbgv`'s own pre-session placeholders (which encoded only a participant id).

10. [Done, CKKS slice - CKG/PCKS/PartialDecryptor only] Mirrored the same three protocols to `mpckks`, closing out the mechanical-port half of items 9/10 for all three schemes (BGV/BFV/CKKS). The underlying algebra is identical to BFV's own mirror (plain, unscaled real-public-key noise), but CKKS introduced two genuine complications neither BGV nor BFV has, both found through research before writing code:

    **Level tracking**: a CKKS ciphertext's own ring shrinks as it gets rescaled (`Evaluator::rescale_next_real` drops RNS components), so `PCKS`/`PartialDecryptor` derive their own working ring via `CkksParams::at_level(ciphertext.level())` rather than assuming the full-level one, truncating each participant's `local_secret_share` (and, for PCKS, the recipient's public key) to match first via repeated `phantom_ring::rns::rescale::drop_last_modulus` - the identical technique `ckks::Decryptor::decrypt_real`/`CkksKeyGenerator::generate_hybrid_relinearization_key_at_level` already use for their own callers. `CollectiveKeyGen` itself needs none of this (key generation happens once, at the top level, before any ciphertext or rescale exists).

    **A small, deliberate `phantom-schemes` API change**: `ckks::Ciphertext::new_real`/`ckks::Plaintext::new_real` were `pub(crate)`, unlike BGV/BFV's own equivalent wrapper types (always `pub`, letting `phantom-multiparty` construct one directly from raw poly data) - widened to `pub`, with each's own doc comment updated to acknowledge `phantom-multiparty`'s collective-decryption/PCKS aggregation as an additional legitimate caller alongside the existing `Encryptor`/`Encoder`/`Decryptor` ones, the same kind of small, purpose-built cross-crate extension `BgvRelinearizationKey::from_rows` already was for GKG's own aggregation (item 4c).

    **A genuine, previously-undocumented CKKS-specific finding, surfaced by a real test failure, not assumed**: unlike BGV/BFV, CKKS has no exact modular reduction stripping smudging noise below a correctness threshold - the decoded value is the raw `(c0+c1*s)/Delta`, so smudging noise shows up *directly* in the result, and since `smudging_std_dev` at the recommended 40-bit statistical target comes out to a *fixed* magnitude independent of scale (it recovers `fresh_public_key_noise_bound`, then multiplies by `2^20`), the scale must be comfortably larger than that fixed floor for a useful decode. Confirmed directly: a first attempt at a 30-bit-scale test fixture decoded values off by 1-10 (completely swamping signals of the same magnitude, not a correctness bug - traced to this precision cost specifically); a 45-bit-scale fixture (two already-verified ~50-bit primes, no rescaling needed since these tests don't rescale) leaves ~17 bits of margin and passes cleanly. `mpckks::reencryption`'s own doc comment carries the full derivation. A real deployment needs to size its own scale with this tradeoff in mind, not just for the usual multiplicative-depth reasons - this also meant `ciphertext_noise_bound` (an explicit, caller-supplied parameter for BGV/BFV) could be *dropped* from CKKS's own `create_share` signatures, derived internally instead from `ciphertext.precision()` via `ckks::noise::noise_bound_from_precision` - CKKS's own `Ciphertext` already tracks a real per-value precision estimate BGV/BFV's don't, a genuine ergonomic improvement CKKS's own design permits rather than an inconsistency.

    `mpckks::wire` needed the same new `encode_poly`/`decode_poly`/`encode_poly_pair`/`decode_poly_pair` helpers `mpbgv`/`mpbfv::wire` already had - its own placeholder shape worked entirely on the transparent `Complex64`-slots wire format, never on raw `Poly` data at all. Also updated the public `phantom-examples::mpckks_basic` example (same compile-break-on-signature-change lesson `mpbgv_basic` already taught this session) to a real two-participant workflow with its own dedicated, appropriately-scaled parameter fixture, moved into `tests/examples_run.rs`'s real-parameters test group.

    Verified end to end (`phase15_mpbfv.rs`, rewriting the file's own three affected placeholder tests plus updating a fourth's call sites): a real collective public key round trip (`CollectiveKeyGen` + real encrypt/decrypt, correctness checked against an independently-summed collective secret), real PCKS delivering a result to a wholly separate, non-DKG recipient keypair, and real `PartialDecryptor` collective decryption needing no test-only secret reconstruction at all (its own `aggregate_plaintext` output already *is* the decrypted plaintext). `RelinearizationKeyGen`/`GaloisKeyGen`/`InteractiveBootstrap` remain untouched placeholders for BFV, deliberately out of this item's own scope. The CKKS mirror of these same three protocols was not attempted in this item either — separate, not-yet-scoped follow-up work.

    **Scope decision**: the check was added directly to `mpbgv`'s own four real `create_share*` methods, not to the shared `common::shares::Share::new` every scheme's protocol type calls (including `mpbfv`/`mpckks`, still transparent placeholders with 12 more `Share::new` call sites total) — changing that shared constructor's own signature would have forced every placeholder call site to thread a guard through too, for protocols that don't produce real key material yet and don't need this protection today. `ReplayGuard` is deliberately a small, explicit, caller-owned tracker (an in-memory `HashSet` keyed on `(SessionId, round, ProtocolKind tag, ParticipantId)`) rather than anything built into `SessionState`/`Share` themselves - one instance belongs to *one participant's own process*, reused across every real `mpbgv` call that participant makes over its lifetime, matching this crate's own existing philosophy of plain, externally-driven types with no hidden I/O or global state. It protects an honest participant from accidentally re-exposing their own key material (e.g. retrying after a crash or network partition); it cannot stop a malicious participant from simply not using one, and the protection only holds across calls made against the *same* `ReplayGuard` instance - a caller whose process restarts must persist and restore this state themselves to keep the protection across restarts.

    Verified via a new, focused test (`phase14_mpbgv.rs::replay_guard_rejects_a_second_share_for_the_same_session_round_and_participant`): a second `create_share` call for the identical session/round/participant is rejected with `MultipartyError::ReplayedShare` before any crypto work runs, while a different participant, round, or session succeeds independently - plus unit tests directly on `ReplayGuard` itself. All prior `mpbgv` integration tests updated to construct and thread one `ReplayGuard` per participant.

11. [Done] Mirrored `GaloisKeyGen`/`RelinearizationKeyGen` to `mpbfv` and `mpckks`, closing out item 9's own deliberately-deferred remainder. Unlike CKG/PCKS/PartialDecryptor (a mechanical port, items 9/10), this genuinely could not be a port of `mpbgv::gkg`/`rkg`: those exist specifically to avoid the RNS-hybrid key-switching technique, since `mod_down`'s rounding error isn't generally a multiple of `t`, corrupting BGV's exact `mod t` decode (`bgv::relinearization`'s own module doc comment). BFV and CKKS have no such constraint, and both already use the identical single-party RNS-hybrid construction unmodified for their own real keys (`phantom_lattice::rlwe::KeyGenerator::generate_hybrid_relinearization_key`/`generate_hybrid_galois_key`, `keyswitch::generate_key_switch_key`/`key_switch`) - so what this item needed was a **new collective version of that construction**, derived once and shared, not two separate ports.

    **Derived and numerically verified in Python (250 GKG+RKG trials across 1-8 parties, plus 200 trials on a separate load-bearing fact) before writing any Rust.** The single-party construction's row `j` (one per `Q`-modulus, no "levels" dimension) is `b_j = P*hat_Q_j*s_old + e_j - a_j*s_new`, computed in the extended `QP` ring. GKG's `s_old = sigma(s) = sum_p(sigma(s_p))` is linear, so each participant contributes additively per row - a direct structural match to `mpbgv::gkg`'s own pattern, with `crt_basis_constant`'s `P*hat_Q_j` scaling in place of `scale_by_base_power_and_lift`, and each participant's own secret rebased into `QP` via the already-`pub` `rebase_ternary_secret` (exact for a ternary value) before the automorphism-then-rebase order `generate_hybrid_galois_key` itself uses. RKG's `s_old = s^2` is quadratic (cross terms `s_p*s_q`, `p != q`, no single participant can form alone), needing the same two-round trick `mpbgv::rkg` already uses - but adapted to `QP`, with **one extra round** BGV's own RKG didn't need: a collective public key over `QP` (BGV's RKG reused its already-existing `Q`-ring collective key directly, since it never leaves `Q`). With that key in hand, round 1 and round 2 mirror `mpbgv::rkg` exactly, but *per row* `j`, with the per-row public scale applied to the *signal* inside round 1, never to already-aggregated noise (confirmed necessary: scaling a finished aggregate afterward would scale its own fresh noise by the same huge factor, destroying the key - each row repeats the full two rounds with independently-sized fresh noise instead). Both constructions rest on a separately-verified fact: summing each participant's own individually-rebased ternary secret into `QP` gives the same result as rebasing the true collective sum directly (small-integer CRT embedding commutes with addition here, the same principle `embed_centered_coeffs` already relies on elsewhere). Ordinary key-generation noise throughout (`STANDARD_ERROR_STD_DEV`), not smudging - this is DKG, not a reveal-to-an-outsider protocol, matching `mpbgv::gkg`/`rkg`'s own convention; `ReplayGuard` applies to every share exactly as it does everywhere else in this crate.

    **Built once, wired twice**: since BFV and CKKS share the identical single-party construction, the collective version lives in one new scheme-agnostic module, `phantom-multiparty::common::hybrid`, operating purely on `phantom_lattice::rlwe::{RlweParams, SecretKey, PublicKey}` and `phantom_ring::Modulus` - no scheme-specific type anywhere. `mpbfv`/`mpckks`'s own `gkg.rs`/`rkg.rs` are thin wrappers supplying their own `RlweParams`/`p_moduli` and handling session/replay/wire concerns, delegating all row arithmetic to `common::hybrid`. This premise was validated directly, not just assumed: both scheme wrappers compiled clean on the first attempt once the shared core was proven, and all BFV/CKKS tests passed with only two minor test-authoring bugs needing fixes (a variable shadowing a same-named fixture function; a CKKS rotation test whose expected value was computed on a partially-populated slot vector instead of the fully slot-count-sized one the real encoder actually produces) - neither a construction bug.

    **A small, deliberate `phantom-lattice` API addition**: `KeySwitchKey::from_rows(qp_params, rows)`, a new `pub const fn` constructor bypassing `generate_key_switch_key` (which needs `s_old` fully known - unusable when the rows are a multiparty protocol's collectively-combined output, with no `s_old` ever assembled anywhere). `GaloisKey::from_key_switch_key`/`RelinearizationKey::from_key_switch_key` needed no changes - both already `pub const fn`, confirming the existing "two-state" (placeholder-or-real) design already supported wrapping a collectively-built `KeySwitchKey` directly.

    Verified end to end (`phase15_mpbfv.rs`/`phase16_mpckks.rs`, replacing each file's own last remaining placeholder test with two real ones): real collective GKG rotating a genuine batched/encoded ciphertext, decoded and compared against a manually-rotated reference; real three-round collective RKG relinearizing a genuine ciphertext product (`evaluator.mul_real`), decoded and compared against the raw unrelinearized product's own decode. This closes out Workstream 7's own BGV-to-BFV/CKKS mirror in full: all five real protocols (CKG, GKG, RKG, PCKS, PartialDecryptor) are now real for BGV, BFV, and CKKS alike. `InteractiveBootstrap` remains an untouched placeholder for all three schemes, independent of this mirror.
...
1. [Done] Added golden-byte fixture tests for every one of the 16 `phantom_utils::serialization::SerializationHeader`-based type pairs (`phantom-lattice`: `PublicKey`/`EvaluationKey`; `phantom-schemes`: all 11 BGV/BFV/CKKS params/plaintext/ciphertext pairs including the CKKS real variants; `phantom-bootstrapping`: `BootstrapParams`; `phantom-circuits`: `PolynomialEvalPlan`/`BsgsPlan`), plus the three `phantom-multiparty` bespoke-format pairs (`Share`, `VssCommitmentSet`, `VssShare`) in a new `tests/serialization.rs`. Each fixture is a previously-generated encoding (via a throwaway script, never hand-computed) asserted in both directions - this is what a pure round-trip test structurally can't catch, since round-tripping only proves this build's own encoder and decoder agree with *each other*, not that the wire shape hasn't silently drifted from what's already out there. Caught a real transcription bug directly: an early hand-typed golden constant for `phantom-schemes` didn't match the actual encoder output, exactly the class of drift this item exists to catch - fixed by regenerating and splicing the constants in programmatically instead of retyping them by hand.
2. [Done] Extended every crate's own domain-mismatch test to cover every type pair (not just one representative pair, the previous state), and added the previously-untested wrong-version case everywhere (same domain, corrupted version bytes) - asserting the specific expected error variant (`UtilsError::InvalidDomain`/`UnsupportedVersion`) rather than a bare `.is_err()`. `phantom-multiparty`'s own three bespoke-format types get the equivalent wrong-tag treatment (no separate version field exists in that format to mismatch).
3. [Done] Added a `proptest` property to every crate with a `serialization.rs` (plus `phantom-multiparty`) asserting every `decode_*` function never panics on arbitrary bytes (`any::<Vec<u8>>()` fed directly in, `proptest` itself treating a panic as a failure) - the direct interpretation of "decode rejection": a decoder must fail gracefully with a typed `Err`, never panic, across a much wider input space than any fixed set of hand-picked malformed payloads.
4. [Done, narrower than the original task] Audited existing coverage before adding anything: `phantom-lattice/tests/randomized.rs` (Workstream 4 item 9) already covers the RLWE/RGSW layer with 300-trial hand-seeded randomized tests across encrypt/decrypt, add/sub, multiplication, relinearization, Galois automorphism, repack, and RGSW external product - a new `phantom-lattice` property-test file would have duplicated it. `phantom-schemes`/`phantom-bootstrapping` similarly already carry extensive hand-seeded randomized coverage from this session's own earlier crypto work. The genuine, confirmed gap was `phantom-circuits`: zero randomized-trial tests anywhere, only fixed hand-picked cases. New `phantom-circuits/tests/property_tests.rs` covers `common::lintrans`'s own scheme-independent planning logic (dense-to-diagonal-and-back round trip, BSGS offset decomposition) across randomly-generated matrices - pure data-structure conversions needing no encryption, exactly where `proptest`'s wider generator-driven input space is cheap and valuable, unlike an already-well-covered encrypted path.
5. [Done] New `docs/internal/serialization-compatibility-policy.md`: what a domain tag vs. a version each mean and when to change which (a new type gets a new domain tag; a changed wire shape for an existing type bumps its version; a domain tag is never reused for an incompatible shape), the current deliberate behavior that `SerializationHeader::read_expected` requires an *exact* match with no forward/backward negotiation, the five-case test requirement from items 1-3 above, and item 6's own policy folded in below.
6. [Done] Confirmed, don't just assume: grepped every crate's own `serialization.rs` and found no `SecretKey`/raw secret scalar has ever had an `encode`/`decode` pair - the invariant already held. Documented it explicitly in the new policy doc rather than leaving it implicit, including the one deliberate, already-reviewed exception: `vss::VssShare` genuinely does serialize a secret value (a participant's own Shamir/Pedersen evaluation), because controlled transmission under commitment verification is VSS's whole purpose, not an oversight of this rule.

Also updated `docs/developer-guide.md`'s own testing-conventions section (three cases → five, `proptest` marked adopted rather than "not yet") and `docs/internal/dependency-policy.md`'s secret-serialization mention, both of which had drifted out of sync with the actual state before this item - the same "user-facing/contributor-facing docs drift silently" lesson already applied to `concepts.md`/`user-guide.md`/`architecture.md` earlier this session.

Exit criteria:

- [Met] Serialization formats are intentionally versioned and regression-tested (golden-byte fixtures + exhaustive domain/version-mismatch tests, items 1-2).
- [Met] Decode rejection is tested beyond simple truncation cases (`proptest` decode-never-panics coverage, item 3).
- [Met] Secret serialization remains an explicit non-default decision (confirmed and documented, item 6).
...
1. [Done] All 10 bench binaries (the 9 groups listed below plus `ring_extend_basis`, added earlier) stayed runnable without extra setup throughout this item - `cargo bench -p phantom-benches` needs no network access beyond the one-time `cargo build` dependency fetch, same as any other dev-dependency.
2. [Done] Criterion approved - this was already pre-approved in `docs/internal/dependency-policy.md` "once Workstream 9 is picked up"; this item executed that decision rather than reopening it.
3. [Done] Added Criterion benchmark groups for all 9 named groups (`ring_ntt`, `ring_rns`, `rlwe_encrypt`, `rlwe_keyswitch`, `bfv_eval`, `bgv_eval`, `ckks_eval`, `ckks_bootstrapping`, `multiparty`), converting every bench binary from the old `std::time::Instant`-based `time_iterations`/`print_result` smoke timing to `criterion_group!`/`criterion_main!`, sharing one CI-tuned `Criterion` configuration (`benches/support/mod.rs::criterion_config` - 500ms warm-up, 1.5s measurement, sample size 10, so the full `cargo bench -p phantom-benches` run stays under a minute despite `make bench` being part of `make ci`) via a `mod support;` shared by every bench file (not `phantom-benches`'s own `src/lib.rs`, since `criterion` is a dev-dependency and code in the crate's own lib target can't see it).
4. [Done, ring/RLWE-level primitives; scheme-level intentionally left single-tier] `ring_ntt`, `ring_rns`, `ring_extend_basis`, `rlwe_encrypt`, and `rlwe_keyswitch` each gained a `toy`/`small` `BenchmarkGroup` (`toy` = the documented development preset, `small` = `N=1024`, the smallest degree the homomorphicencryption.org 128-bit-security table covers) - scaling these needs only a modulus/degree choice, no new scheme parameters. Found a real overflow while building `ring_rns`'s `small` tier: `reconstruct_residue` accumulates its basis product with a checked `u128` multiplication, but its *per-term* reconstruction (`residue * partial * inv`, unchecked, only reduced mod the product at the end) can itself overflow `u128` for large moduli well before the product does - true even in release mode (silent wraparound, a wrong answer) and caught as a debug-mode overflow panic once `cargo test` exercised the bench in test mode. Fixed by using the same already-validated `~22`-bit modulus family `rlwe_keyswitch`'s own toy tier uses instead of inventing a new large-prime pair, keeping `reconstruct_residue` safely inside the "small parameter sets" scope its own doc comment says it's for. The remaining five (`bfv_eval` through `multiparty`) deliberately stay single-tier at their existing, already-reviewed toy preset (still calling the same `phantom_examples::*` workflow functions as before): constructing a second, larger BGV/BFV/CKKS/bootstrapping/multiparty parameter set is a parameter-preset design decision, not a benchmarking one, and stays gated the same way `docs/user-guide.md#choosing-parameters` already states there is currently no production preset to graduate to - this item didn't reopen that decision, and no `N≥1024` multi-modulus scheme preset existed anywhere in the codebase to reuse.
5. [Done] New `phantom_benches::alloc::CountingAllocator` (`crates/phantom-benches/src/lib.rs`, no `criterion` dependency needed here so it stays in the shared lib crate) - a `std::alloc::System`-wrapping `#[global_allocator]` with atomic call-count/byte counters. Each of the five single-tier evaluator benches installs it and prints a one-shot count (`phantom_benches::alloc::print_one_shot`, e.g. `bfv_eval allocations: 170 calls, 13768 bytes`) around one representative call, labeled explicitly as a diagnostic rather than a Criterion-averaged metric, since Criterion's public API has no per-iteration allocation hook without a custom `Measurement` implementation - more machinery than a "where possible" diagnostic needs.

Also updated `docs/technical-manual.md`'s Performance section (the new Criterion configuration, the toy/small tiers and why the five scheme-level benches don't get a second tier, the allocation-count diagnostic), `docs/internal/dependency-policy.md`'s `criterion` bullet (pre-approved → adopted, mirroring how the `proptest` bullet reads after Workstream 8), `docs/developer-guide.md`'s "adding a benchmark target" shape, and `README.md`'s now-inaccurate "dependency-free smoke benchmarks" crate-table line.

Exit criteria:

- [Met] Performance regressions are visible before release - Criterion saves a baseline per benchmark and reports a statistically-grounded change/regression verdict on every subsequent run, not just a raw number.
- [Met] Benchmark documentation explains which numbers are smoke signals versus meaningful performance baselines - `docs/technical-manual.md#performance` now states plainly that every number is still measured on development-scale (toy/small) presets, not production-representative ones, and distinguishes Criterion's statistically-sampled timings from the evaluator benches' one-shot allocation-count diagnostic.

### Workstream 10 - Alpha Release Checklist

Before tagging the next release:

1. [Done] `make ci` (`cargo fmt --all -- --check`, `cargo test --workspace --all-targets`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo doc --workspace --no-deps`, `cargo bench -p phantom-benches`) - the exact command matrix this item lists - passes clean.
2. [Done] `make examples` runs all 16 `phantom-examples` binaries (a superset of the 5 representative ones originally listed here) end to end without error.
3. [Done] Audited README.md, `docs/internal/implementation-plan.md`, and `docs/internal/release-checklist.md` against each other and against actual repo state - found and fixed one real discrepancy: `release-checklist.md`'s own Beta section still listed "decide whether Criterion is part of the benchmark dependency policy... not yet adopted", "audit serialization compatibility", and "confirm secret-bearing types remain non-serializable" as open TODOs, even though Workstreams 8 and 9 had already closed all three. Everything else (README's Status section, the roadmap phase table, the Alpha milestone list, the Alpha section of the release checklist) was already internally consistent.
4. [Done] Audited every toy/scaffold warning across `SECURITY.md`, `docs/concepts.md`, `docs/technical-manual.md`, `docs/architecture.md`, `docs/user-guide.md`, and `docs/getting-started.md` for staleness (both directions: a claimed gap that's actually closed, and an undocumented real gap) - grepped `crates/*/src` for `unimplemented!`/`todo!` to confirm no undocumented placeholders exist. Found and fixed two real, contradictory leftovers predating this session's noise-wiring work: `docs/concepts.md` claiming "no scheme wires [the real discrete Gaussian] into encryption noise yet" (false since Workstream 4 item 1 - every real, non-transparent encryption/keygen path samples genuine error with it), and `docs/technical-manual.md` claiming "`phantom-lattice::rlwe::Encryptor` samples no error term at all yet" in the very same document that describes the real sampled error six lines later. `SECURITY.md` itself, and every other file audited, were already accurate.
5. [Done] New root `CHANGELOG.md` (Keep-a-Changelog-style, one `0.1.0 - Alpha` entry): implemented crate map, the 16 supported examples, a condensed security-status summary (pointing to `SECURITY.md`/`technical-manual.md` for the full account), known scaffold limitations, and the planned production-hardening tracks pulled from this file's own remaining open items (real-path migration for `phantom-circuits`/`phantom-bootstrapping`/`phantom-multiparty`, constant-time sampling, BFV/CKKS relinearization noise bounds, RGSW-consuming bootstrapping, BGV/BFV bootstrapping, multiparty active-security review, and the two items already intentionally deferred - Workstream 2's production-preset split and Workstream 3 item 5b's lazy-reduction NTT). Linked from README.md's Status section.

Exit criteria:

- [Met] The full local command matrix and every example run clean, matching what CI already runs on every push/PR.
- [Met] README, the implementation plan, and the release checklist agree with each other and with actual repo state.
- [Met] Every toy/scaffold/security warning across the docs is accurate as of this item, not stale in either direction.
- [Met] `CHANGELOG.md` exists with the required five sections for the current (Alpha) release.
