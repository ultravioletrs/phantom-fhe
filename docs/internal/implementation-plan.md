...
4a. Wire the real NTT into `Ring::schoolbook_mul`'s callers (or replace it outright) so ciphertext multiplication above `phantom-ring` actually gets the O(N log N) benefit - today the fast transform exists but nothing above `phantom-ring` calls it for multiplication.
