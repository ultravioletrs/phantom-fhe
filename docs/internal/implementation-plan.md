...
- The wraparound-integer bound `EvalMod` relies on is derived, not assumed. **Met** - and the derivation surfaced that `EvalMod`'s own domain doesn't yet cover a realistic secret (addressed by item 5's own domain-widening), which in turn surfaced the DFT-amplification and complex-wraparound gaps above.

**Not met, tracked as a further, separate item**: `Bootstrapper::bootstrap_real_wide` running end to end starting from a genuinely-exhausted real ciphertext (not a test-constructed, engineered one) - blocked on extending `EvalMod` to reduce a genuinely complex slot-domain wraparound (real and imaginary parts independently, e.g. via `conjugate_real` extraction first, the direction `reduce_mod_q_real`'s own doc comment already points at), not merely a real one at a wider domain. Revisit once that derivation is done.
