...
Current status: Alpha Hardening Workstream 11 (CKKS real bootstrapping: modulus raise) landed a real, tested `Evaluator::raise_level_real`, a widened real-domain `EvalMod::reduce_mod_q_real_wide`, and a genuinely complex-valued `EvalMod::reduce_mod_q_real_complex_wide` - each independently verified. Chaining them together on a real, genuinely-exhausted ciphertext still doesn't work, but the reason is now understood precisely (not just bounded from outside): `raise_level_real` perturbs a ciphertext's raw ring-polynomial coefficients (true RLWE semantics), while `CoeffsToSlots::apply_real` (this repository's own scaffold convention, established in Workstream 6 item 5) applies a generic DFT to whatever the ciphertext's canonical-embedding *slots* already decode to - two different, currently-incompatible notions of "coefficient domain." Reconciling them (most likely by rebuilding `CoeffsToSlots`/`SlotsToCoeffs` around the encoder's own `5^j`-power embedding rather than a generic DFT) is a materially larger redesign than the domain-width/complex-value work just finished, tracked as Workstream 11's own remaining item - see its own entry for the full derivation. Workstreams 1 (Phase 0 cleanup), 2 (scaffold boundary and API honesty), 3 (ring and RNS foundation hardening, item 5b intentionally deferred, see its own entry), 4 (production RLWE/RGSW track), 5 (production BFV/BGV/CKKS track), and 6 are done - see below. Workstream 6's own item 1 (keeping circuit planning APIs stable) is an ongoing invariant, not a one-time task, and stays in force going forward the same way analogous "stay stable" items do in earlier workstreams. Workstream 7 (multiparty and protocol security) is next once Workstream 11's remaining item is either finished or explicitly deferred.
...
- The wraparound-integer bound `EvalMod` relies on is derived, not assumed. **Met** - and the derivation surfaced that `EvalMod`'s own domain doesn't yet cover a realistic secret (addressed by item 5's own domain-widening) or a genuinely complex value (addressed by item 6's own complex extension), which in turn surfaced the deeper `CoeffsToSlots`/`raise_level_real` domain-mismatch below.

**Not met, tracked as a further, separate item**: `Bootstrapper::bootstrap_real_wide`/`bootstrap_real_complex_wide` running end to end starting from a genuinely-exhausted real ciphertext (not a test-constructed, engineered one) - blocked on reconciling `CoeffsToSlots::apply_real`'s own generic-DFT-over-decoded-slots convention with `raise_level_real`'s own raw-ring-coefficient semantics (item 6's own finding above), a materially larger redesign than the domain-width/complex-value gaps items 5/6 already closed. Revisit once that reconciliation is designed.
